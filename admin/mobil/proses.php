...
if ($_GET['aksi'] == 'tambah') {
    ...
    if(!in_array($filetype, array_keys($allowedTypes))) {
        ...
        if (move_uploaded_file($tmp_name, $target_path)) {
            ...
            $sql = "INSERT INTO `tbl_mobil_Fsaylend_Lykta`(`no_plat`, `merk`, `harga`, `deskripsi`, `status`, `gambar`) 
                VALUES (?,?,?,?,?,?)";
...
if ($_GET['aksi'] == 'edit') {
    ...
    $data[] = $id;
    $sql = "UPDATE tbl_mobil_Fsaylend_Lykta SET no_plat= ?, merk=?, harga=?, deskripsi=?, status=?, gambar=?
        WHERE id_mobil = ?";
    $row = $koneksi->prepare($sql);
    $row->execute($data);
...
if (!empty($_GET['aksi'] == 'hapus')) {
    ...
    $sql = "DELETE FROM tbl_mobil_Fsaylend_Lykta WHERE id_mobil = ?";
    $row = $koneksi->prepare($sql);
