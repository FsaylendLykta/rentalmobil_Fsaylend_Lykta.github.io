...
if($_GET['id'] == 'login'){
    ...
    $row = $koneksi->prepare("SELECT * FROM tbl_user_Fsaylend_Lykta WHERE username = ? AND password = md5(?)");
...
if($_GET['id'] == 'daftar')
{
    ...
    $row = $koneksi->prepare("SELECT * FROM tbl_user_Fsaylend_Lykta WHERE username = ?");
    ...
    else
    {
        ...
        $sql = "INSERT INTO `tbl_user_Fsaylend_Lykta`(`nama_pengguna`, `username`, `password`, `level`)
                VALUES (?,?,?,?)";
...
if($_GET['id'] == 'booking')
{
    ...
    $sql = "INSERT INTO tbl_rental_Fsaylend_Lykta (kode_booking, 
    id_login, 
    id_mobil, 
    ktp, 
    nama, 
